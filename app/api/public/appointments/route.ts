...
export async function POST(req:Request){
 try{
  ...
  const [{data:biz},{data:svc},{data:sub}]=await Promise.all([
   ...
   db.from('services').select('price_cents,duration_minutes,buffer_minutes').eq('id',serviceId).eq('business_id',businessId).single(),
  ...
  if(!biz||biz.access_status==='blocked'||biz.subscription_status!=='active'||!paid||Date.now()>paid+GRACE||!svc)return NextResponse.json({error:'Agenda indisponível.'},{status:403});
  const start=new Date(startsAt);if(start.getTime()<Date.now()+Math.max(60,biz.min_notice_minutes||60)*60000)return NextResponse.json({error:'Escolha um horário com pelo menos 1 hora de antecedência.'},{status:409});
  ...
  if(!customer){const created=await db.from('customers').insert({business_id:businessId,name,phone}).select('id').single();if(created.error)throw created.error;customer=created.data}else await db.from('customers').update({name}).eq('id',customer.id);
